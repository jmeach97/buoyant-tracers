module npcmodel
    ...
    contains
        ...
        subroutine PASSIVE_TRACER_INIT(this,dt,sqrt_n_part,mu,lambda,N0,C0,init_type)
            Class(PassiveTracers):: this
            real(kind=c_double):: dt,mu,lambda,N0,C0
            real(kind=c_double),dimension(:),allocatable:: gauss_points
            character(len=100):: nums
            character(len=*):: init_type
            ...
            path3='/rds/general/user/jom20/home/FINAL_LAGRANGIAN/&
            &codebase/pollutecode/GAUSSIANELIM/GaussLegPoints'&
            ...
            SELECT CASE(init_type)
                ...
                CASE('grid')
                    do i=1,sqrt_n_part
                        gauss_points(i)=real(i-1,kind=c_double)/sqrt_n_part
                    end do
            END SELECT
            ...
            call this%particle_manager%create_parameter('C0',C0)
            ...
            positions(6)=this%particle_manager%get_pos('C')
            do i=1,sqrt_n_part
                do j=1,sqrt_n_part
                    ...
                    this%particle_manager%attribute_values(i+sqrt_n_part*(j-1),positions(4))=N0-lambda/mu - C0
                    this%particle_manager%attribute_values(i+sqrt_n_part*(j-1),positions(5))=(lambda/mu)*N0/(N0-C0)
                    this%particle_manager%attribute_values(i+sqrt_n_part*(j-1),positions(6))=-1*(lambda/mu)*(1-N0/(N0-C0))
        ...
        function PASSIVE_UPDATE_FUNCTION(this,velocity_instance)result(step_val)
            ...
            &-this%get_param('lambda')*this%get('P')-this%get_param('mu')*this%get('C')*this%get('P')
            ...
            & - this%get_param('mu')*this%get('C')*this%get('P')&
            &-this%get_param('lambda')*this%get('C')+this%get_param('lambda')*this%get_param('C0')/this%get('A')
